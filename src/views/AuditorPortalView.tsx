  ...
  if (tokenStatus === 'missing') {
    return (
      <div style={{ padding: 32, maxWidth: 640, margin: '40px auto' }}>
        <h1 style={{ fontSize: 18, fontWeight: 600 }}>{t('No token', 'No token')}</h1>
        ...
        <>
          <section style={{ marginBottom: 18, padding: 16, background: 'var(--color-background-primary)', borderRadius: 8, border: '0.5px solid var(--color-border-secondary)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              <div>
                <div style={{ fontSize: 11, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: 0.4 }}>{t('Engagement', 'Engagement')}</div>
                <div style={{ fontSize: 16, fontWeight: 600, marginTop: 2 }}>
                  {t('Tenant:', 'Tenant:')} <code style={{ fontSize: 14, fontFamily: 'var(--font-mono, monospace)' }}>{exec.tenant_id}</code>
                  {exec.framework_filter ? <> {t('· Scope:', '· Scope:')} <code style={{ fontSize: 14 }}>{exec.framework_filter}</code></> : null}
                </div>
                <div style={{ fontSize: 11, color: 'var(--color-text-tertiary)', marginTop: 4 }}>
                  {t('Generated', 'Generated')} {exec.generated_at}
                </div>
              </div>
              <button
              ...
              >
                {t('Download signed audit pre-packet', 'Download signed audit pre-packet')}
              </button>
            </div>
          </section>
          ...
          <section style={{ marginBottom: 18 }}>
            <h2 style={{ fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4, color: 'var(--color-text-tertiary)', marginBottom: 8 }}>
              {t('Headline KPIs', 'Headline KPIs')}
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 10 }}>
              <KPI label={t('Composite risk score', 'Composite risk score')} value={`${(exec.kpis.composite_risk_score * 100).toFixed(1)}%`} sub={exec.kpis.composite_risk_status} tone={statusTone(exec.kpis.composite_risk_status)} />
              <KPI label={t('Critical open gaps', 'Critical open gaps')} value={String(exec.kpis.open_critical_gaps)} tone={exec.kpis.open_critical_gaps > 0 ? 'red' : 'green'} />
              <KPI label={t('Overdue remediation', 'Overdue remediation')} value={String(exec.kpis.open_overdue_tasks)} tone={exec.kpis.open_overdue_tasks > 0 ? 'amber' : 'green'} />
              <KPI label={t('Mean remediation', 'Mean remediation')} value={exec.kpis.mean_remediation_days > 0 ? `${exec.kpis.mean_remediation_days}d` : '—'} />
              <KPI label={t('Evidence freshness', 'Evidence freshness')} value={`${exec.kpis.evidence_freshness_pct.toFixed(1)}%`} tone={exec.kpis.evidence_freshness_pct >= 80 ? 'green' : exec.kpis.evidence_freshness_pct >= 50 ? 'amber' : 'red'} />
            </div>
          </section>
          ...
          {heatmap && heatmap.cells.length > 0 ? (
            <section style={{ marginBottom: 18, padding: 16, background: 'var(--color-background-primary)', borderRadius: 8, border: '0.5px solid var(--color-border-secondary)' }}>
              <h2 style={{ fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4, color: 'var(--color-text-tertiary)', marginBottom: 8 }}>
                {t('Risk heatmap ·', 'Risk heatmap ·')} {heatmap.total_open}open
                              </h2>
              ...
              <div style={{ fontSize: 10, color: 'var(--color-text-tertiary)', marginTop: 10 }}>{t(
                'Open + in-treatment risks. Score = likelihood × impact (1..16).',
                'Open + in-treatment risks. Score = likelihood × impact (1..16).'
              )}</div>
            </section>
          ) : null}
          ...
          <section>
            <h2 style={{ fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4, color: 'var(--color-text-tertiary)', marginBottom: 8 }}>
              {t('Per-framework posture', 'Per-framework posture')}
            </h2>
                  ...
                  <div key={fw.framework_id} style={{ marginTop: 12, padding: 16, background: 'var(--color-background-primary)', borderRadius: 8, border: '0.5px solid var(--color-border-secondary)' }}>
                    ...
                    {fw.yaml_sha256 ? (
                      <div style={{ marginTop: 10, fontSize: 10, color: 'var(--color-text-tertiary)' }}>
                        {t('Control set:', 'Control set:')} {fw.yaml_version ? `v${fw.yaml_version} · ` : ''}{t('sha256:', 'sha256:')}{fw.yaml_sha256.slice(0, 8)}…{fw.yaml_sha256.slice(-8)}
                      </div>
                    ) : null}
                  </div>
                );
