  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <PageTitle>{t('Policy Tasks', 'Policy Tasks')}</PageTitle>
        <Button onClick={loadTasks} disabled={loading}>
          {loading ? 'Refreshing...' : 'Refresh'}
        </Button>
      </div>
      {error ? <ErrorBox title={t('Policy tasks error', 'Policy tasks error')} detail={error.message} /> : null}
      {message ? <div className="rounded-md border border-[#274266] bg-[#0f1f36] p-3 text-sm">{message}</div> : null}
      <Card>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-sm font-semibold text-slate-100">{t('Role:', 'Role:')} {roleLabel}</div>
          <div className="text-xs text-slate-400">{t(
            'Auditors can create and manage tasks.',
            'Auditors can create and manage tasks.'
          )}</div>
        </div>
        {canManage ? (
          <>
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              <div>
                <Label>{t('Policy key', 'Policy key')}</Label>
              ...
              <div>
                <Label>{t('Reminders (ISO, comma separated)', 'Reminders (ISO, comma separated)')}</Label>
            ...
            <div className="mt-4">
              <Button onClick={handleCreate}>{t('Create task', 'Create task')}</Button>
            </div>
          </>
        ) : isReadOnlyAuditor ? (
          <div className="mt-3 text-sm text-slate-300">{t(
            'Read-only auditors can view tasks but cannot edit.',
            'Read-only auditors can view tasks but cannot edit.'
          )}</div>
        ) : (
          <div className="mt-3 text-sm text-slate-300">{t(
            'Collaborators can view tasks but cannot edit.',
            'Collaborators can view tasks but cannot edit.'
          )}</div>
        )}
      </Card>
      <Card>
        <div className="flex flex-wrap items-center gap-4">
          <div className="text-sm font-semibold text-slate-100">{t('Filter', 'Filter')}</div>
      ...
      <div className="space-y-4">
        {items.length === 0 && !loading ? <div className="text-sm text-slate-400">{t('No policy tasks yet.', 'No policy tasks yet.')}</div> : null}
        {items.map((item) => {
          const {
            t
          } = useI18n();
            ...
            <Card key={item.task_id}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-xs text-slate-500">{t('Policy:', 'Policy:')} {item.policy_key || 'n/a'}</div>
                </div>
              ...
              {isExpanded ? (
                <div className="mt-4 space-y-4 border-t border-[#1f365a] pt-4">
                  ...
                  <div>
                    <Label>{t('Reminders', 'Reminders')}</Label>
                        ...
                        item.reminders.map(reminder => {
                          ...
                          return (
                            <div key={reminder.remind_at}>
                              {reminder.remind_at} {t('· last sent:', '· last sent:')} {formatDate(reminder.last_sent_at)}
                            </div>
                          );
                        })
                      ) : (
                        <div className="text-slate-400">{t('No reminders configured.', 'No reminders configured.')}</div>
                      )}
                    </div>
                  </div>
                  ...
                  {canManage ? (
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        <Label>{t(
                          'Update reminders (ISO, comma separated)',
                          'Update reminders (ISO, comma separated)'
                        )}</Label>
                        <Input
                          value={reminderDraft}
